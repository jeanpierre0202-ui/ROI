...
# Pick ONE provider key. Free options first; Anthropic is the paid fallback.
# The dossier is grounded on the free GDELT headlines we already fetch, so no
# paid web search is needed.
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "").strip()          # FREE — aistudio.google.com
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "").strip()              # FREE — console.groq.com
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "").strip()  # FREE models — openrouter.ai
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY", "").strip()    # PAID fallback
...
# live web search is Anthropic-only + paid; off by default. Free providers use GDELT headlines.
...
# Consensus/conviction is computed for ALL names for free; dossiers use the LLM.
try:
